...
const combineBlocks = (blocks, filterStart, filterEnd, project_rates) => {
  ...
  blocks.forEach((block) => {
    ...
    if (
      (!filterStart || blockEnd > filterStart) &&
      (!filterEnd || blockStart < filterEnd)) {
      if (!sums[block.project]) {
        sums[block.project] = 0
      }
      if (filterStart && blockStart < filterStart) { blockStart = filterStart }
      if (filterEnd && blockEnd > filterEnd) { blockEnd = filterEnd }
  ...
  let totals = Object.keys(sums).map((key) => {
    sum = sums[key]
    rate = getProjectRate(key, project_rates)
    return {
      project: key,
      minutes: sum,
      hours: sum/60,
...
const getProjectRate = (project_name, project_rates) => {
  project_name = project_name.replace(/[\[\]]/g, '')
  if (project_rates[project_name] !== undefined) {
    return project_rates[project_name];
  }
  return project_rates['default'] || 0
}
